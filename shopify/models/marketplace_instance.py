...
class MkInstance(models.Model):
    ...
    default_pos_customer_id = fields.Many2one('res.partner', string='Default POS Customer',
                                              domain="['|', ('company_id', '=', False), ('company_id', '=', company_id), ('customer','>', 0)]",
    ...
    def shopify_set_default_pos_customer(self):
        if not self.default_pos_customer_id:
            ...
            pos_customer_id = partner_obj.search(
                ['|', ('company_id', '=', False), ('company_id', '=', self.company_id.id), ('name', '=', 'POS Customer ({})'.format(self.name)), ('customer', '>', 0)])
            if not pos_customer_id:
                partner_vals = {'name': 'POS Customer ({})'.format(self.name), 'customer': 1}
    ...
    def connection_to_shopify(self):
        ...
        shop_url = "{scheme}://{api_key}:{password}@{shop_url}/admin/api/2021-04".format(api_key=self.api_key, password=self.password, scheme=parsed_url.scheme,
