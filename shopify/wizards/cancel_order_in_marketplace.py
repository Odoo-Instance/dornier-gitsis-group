...
class MKCancelOrder(models.TransientModel):
    ...
    def create_shopify_refund_in_odoo(self, order_id):
        invoice_id = order_id.invoice_ids.filtered(lambda x: x.type == 'out_invoice' and x.state == 'paid')
        if invoice_id:
...
class ShopifyRefundPaymentLines(models.TransientModel):
    ...
    available_qty = fields.Float(string='Available Quantity', digits=dp.get_precision('Product Unit of Measure'), required=True, default=1.0)
    to_refund_qty = fields.Float(string='To Refund Quantity', digits=dp.get_precision('Product Unit of Measure'), required=True, default=1.0)
    price_unit = fields.Float('Original Unit Price', required=True, digits=dp.get_precision('Product Price'), default=0.0)
    refund_price_unit = fields.Float('Refund Unit Price', required=True, digits=dp.get_precision('Product Price'), default=0.0)
