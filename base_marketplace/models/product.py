...
class ProductTemplate(models.Model):
    ...
    def _get_weight_uom_id_from_ir_config_parameter(self):
        ...
        if product_weight_in_lbs_param == '1':
            return self.env.ref('product.product_uom_lb', False) or self.env['product.uom'].search([('measure_type', '=' , 'weight'), ('uom_type', '=', 'reference')], limit=1)
        else:
            return self.env.ref('product.product_uom_kgm', False) or self.env['product.uom'].search([('measure_type', '=' , 'weight'), ('uom_type', '=', 'reference')], limit=1)
    ...
    def create_or_update_listing_image(self, listing_id):
        ...
        image_hex = hashlib.md5(self.image).hexdigest()
        listing_image_id = listing_image_obj.search([('image_hex', '=', image_hex), ('mk_listing_id', '=', listing_id.id)], limit=1)
        if listing_image_id:
            return True
        listing_image_obj.create({'name': listing_id.name, 'image': self.image, 'mk_listing_id': listing_id.id, })
        return True
...
class ProductProduct(models.Model):
    ...
    def create_or_update_listing_image(self, listing_item_id):
        ...
        listing_image_obj.create(
            {'name': listing_item_id.name, 'image': self.image, 'mk_listing_id': listing_item_id.mk_listing_id.id, 'mk_listing_item_ids': [(6, 0, listing_item_id.ids)]})
        return True
